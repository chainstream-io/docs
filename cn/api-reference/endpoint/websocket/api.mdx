...
首先需要创建 WebSocket 连接。WebSocket 连接需要在 URL 中通过 `token` 查询参数进行认证。
...
<Tabs>
  <Tab title="使用 SDK（推荐）">
    ...
    <Note>
    调用 `subscribeXXX` 方法时，SDK 会自动检测连接状态，如果未连接会自动建立连接，无需手动调用 `connect()`。
    </Note>
    ...
    - **Rust**: 添加 `chainstream-sdk` 到 `Cargo.toml` ([crates.io](https://crates.io/crates/chainstream-sdk))
  </Tab>
  <Tab title="使用原生 WebSocket">
    使用浏览器原生 WebSocket 或其他 WebSocket 库时，需要在 URL 中拼接 `token` 参数：
    ...
    <Note>
    通过 URL 参数传递 token，浏览器原生 WebSocket 和各种第三方库均可使用，无需特殊处理请求头。
    </Note>
  </Tab>
  <Tab title="命令行测试">
    使用 `wscat` 进行测试：
...
<ParamField query="filter" type="string" required={false}>
  可选的过滤条件，使用Google Common Expression Language (CEL)表达式语言语法。例如：liquidityInUsd > 10000
</ParamField>
...
<ParamField query="filter" type="string" required={false}>
  可选的过滤条件，使用Google Common Expression Language (CEL)表达式语言语法。例如：liquidityInUsd > 50000
</ParamField>
