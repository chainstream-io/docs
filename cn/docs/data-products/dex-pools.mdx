...
<CardGroup cols={2}>
  <Card title="REST" icon="database" href="/cn/api-reference/endpoint/data/dexpool/v2/dexpools-chain-pooladdress-get">
  ...
  <Card title="WebSocket" icon="bolt" href="/cn/api-reference/endpoint/websocket/api">
    `dex-pool-balance:{chain}_{poolAddress}` 实时池流动性；池创建事件从 Kafka `{chain}.dex.pools` 消费。SDK：`client.stream.subscribeDexPoolBalance`。
  </Card>
