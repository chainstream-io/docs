...
适用于拥有链上钱包并通过 [x402 支付](/cn/docs/platform/billing-payments/x402-payments) 购买了订阅的 AI Agent。使用 **Sign-In with X (SIWX)** 标准（EVM 为 EIP-4361，Solana 为等效协议）。
...
<Note>
SIWX 认证需要有效的 x402 订阅。如果订阅已过期，请求将被拒绝。参见 [x402 支付](/cn/docs/platform/billing-payments/x402-payments) 了解如何购买订阅。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="什么是 402 Payment Required 响应？">
    当没有找到有效凭证且启用了 x402 时，网关返回 HTTP 402 并附带购买订阅的指引（`/x402/purchase`）。这使 AI Agent 能够自动购买访问权限。参见 [x402 支付](/cn/docs/platform/billing-payments/x402-payments)。
  </Accordion>
