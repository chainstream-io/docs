...
<Info>
ChainStream 目前支持 **Solana**（`sol`）、**Ethereum**（`eth`）和 **BSC**（`bsc`）。支持的 DEX 包括 Jupiter、Raydium、PumpFun、Moonshot、Candy（Solana）以及 KyberSwap（Ethereum/BSC）。以下部分示例引用了其他协议用于概念说明，请查看[支持的链](/cn/docs/supported-chains)了解当前覆盖范围。
</Info>
...
借贷、收益聚合器与流动性质押是 DeFi 中常见的监控对象。ChainStream 当前已索引的互换与 DEX 分析能力主要覆盖上表中的协议，链为 **Solana**、**Ethereum**、**BSC**。在实现借贷或金库类告警前，请结合 API 参考与[支持的链](/cn/docs/supported-chains)确认实际可用能力。
...
<CardGroup cols={2}>
  <Card title="套利扫描器" icon="magnifying-glass-dollar" href="/cn/docs/tutorials/build-arbitrage-scanner">
    实战构建套利扫描工具
  </Card>
  <Card title="价格预警机器人" icon="bell" href="/cn/docs/tutorials/build-price-alert-bot">
    构建价格预警系统
  </Card>
</CardGroup>
