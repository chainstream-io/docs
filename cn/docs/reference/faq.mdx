...
<AccordionGroup>
  <Accordion title="如何获取 Access Token？">
    ...
    详细步骤见 [认证文档](/cn/docs/platform/authentication/api-keys-oauth)。
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="支持哪些区块链？">
    ...
    完整列表见 [实时数据概念](/cn/docs/access-methods/websocket)。
  </Accordion>
...
<AccordionGroup>
  <Accordion title="收到 429 或 403 错误怎么办？">
    ...
    **排查步骤：**
    ...
    1. **429 - 检查是否超频**
       - Free 套餐：10 请求/分钟
       - 付费套餐：见 [API 安全](/cn/docs/platform/security/api-security)
...
```
    ...
    常见错误码见 [Error Codes](/cn/docs/reference/error-codes)。
  </Accordion>

  <Accordion title="如何调试 API 请求？">
    **方法 1：使用 API Playground**

    在 [API Reference](/cn/api-reference/overview) 页面使用 "Try It" 功能，无需编写代码即可测试。
...
<AccordionGroup>
  <Accordion title="KYT 和 KYA 有什么区别？">
    ...
    详见 [Security Compliance 文档](/cn/docs/compliance/overview)。
  </Accordion>
...
<AccordionGroup>
  <Accordion title="什么是 MCP？">
    ...
    详见 [MCP Server 文档](/cn/docs/ai-agents/mcp-server/introduction)。
  </Accordion>
  ...
  <Accordion title="如何在 Claude Desktop 中使用 ChainStream？">
    ...
    详细配置步骤见 [Claude Integration 指南](/cn/docs/ai-agents/mcp-server/setup)。
  </Accordion>
