...
<Info>
ChainStream 目前支持 **Solana**（`sol`）、**Ethereum**（`eth`）和 **BSC**（`bsc`）。支持的 DEX 包括 Jupiter、Raydium、PumpFun、Moonshot、Candy（Solana）以及 KyberSwap（Ethereum/BSC）。以下部分代码示例引用了其他 DEX 用于概念说明，请查看[支持的链](/cn/docs/supported-chains)了解当前覆盖范围。
</Info>
...
<CardGroup cols={3}>
  ...
  <Card title="多链扫描" icon="layer-group">
    在 Solana 多 DEX 之外，增加 **eth** / **bsc** 的 KyberSwap 报价
  </Card>
...
<CardGroup cols={2}>
  <Card title="DeFi 监控概述" icon="landmark" href="/cn/docs/recipes/defi-monitoring">
    了解 DeFi 监控维度
  </Card>
