...
<Tip>
将 `TOKEN_ADDRESS` 替换为实际代币 mint 地址（例如 Solana 上 USDC 为 `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`）。可按名称查地址时参见 [代币元数据](/cn/graphql/examples/ohlc-stats#how-do-i-look-up-token-information)。
</Tip>
...
<Tip>
若需要可靠的代币价格时间序列，可考虑改用 [Pairs](/cn/graphql/examples/ohlc-stats#how-do-i-get-k-line-ohlc-candlestick-data) Cube — 提供预计算的 K 线数据，含每分钟 open/high/low/close。
</Tip>
