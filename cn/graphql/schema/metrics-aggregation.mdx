...
| **DWM Cube**（如 `OHLC`、`TokenTradeStats`） | 标准时间序列、OHLC 图、成交量随时间 | 快 — 读取预计算的分钟级汇总 |
...
<AccordionGroup>
  <Accordion title="需要蜡烛图">
    使用 **OHLC**（DWM）。已按分钟预计算 open/high/low/close/volume，无需自己对 DEXTrades 聚合。
  </Accordion>
  <Accordion title="需要按 token 的买卖笔数">
    使用 **TokenTradeStats**（DWM）。已按分钟预聚合笔数、成交量与独立交易者。
  </Accordion>
