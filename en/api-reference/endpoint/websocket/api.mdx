...
<Tabs>
  <Tab title="Using SDK (Recommended)">
    ...
    - **Rust**: Add `chainstream-sdk` to your `Cargo.toml` ([crates.io](https://crates.io/crates/chainstream-sdk))
  </Tab>
  <Tab title="Using Native WebSocket">
    When using browser native WebSocket or other WebSocket libraries, append the `token` parameter to the URL:
...
Get real-time max liquidity info of a token in a single pool.
...
Get real-time total liquidity info of a token across all pools.
...
Get real-time bonding curve progress for newly listed tokens.
