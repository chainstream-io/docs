...
For AI agents with on-chain wallets that have purchased a subscription via [x402 payment](/en/docs/platform/billing-payments/x402-payments). Uses the **Sign-In with X (SIWX)** standard (EIP-4361 for EVM, equivalent for Solana).
...
<Note>
SIWX authentication requires an active x402 subscription. If the subscription has expired, the request will be rejected. See [x402 Payment](/en/docs/platform/billing-payments/x402-payments) for purchasing a subscription.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="What is the 402 Payment Required response?">
    When no valid credentials are found and x402 is enabled, the gateway returns HTTP 402 with instructions to purchase a subscription at `/x402/purchase`. This enables AI agents to auto-purchase access. See [x402 Payment](/en/docs/platform/billing-payments/x402-payments).
  </Accordion>
