...
class PriceMonitor {
  ...
  subscribeToken(token) {
    ...
    console.log(`📡 Subscribed to ${token.symbol} price stream`);
  }
...
⚡ Threshold: ${token.thresholdPercent}%
    `.trim();

    await sendAlert(message);
    console.log(`🚨 Alert sent: ${token.symbol} ${change >= 0 ? '+' : ''}${change.toFixed(2)}%`);
  }
}
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries - 1) {
        ...
        console.error(`❌ Notification failed after ${maxRetries} retries`);
        return false;
      }
    }
  }
...
📡 Subscribed to EXAMPLE price stream
📡 Subscribed to SOL price stream
```
