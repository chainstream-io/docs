...
This returns only DEX protocols that had **more than 100 trades** today — protocols with fewer trades are excluded from the result.
...
| **DWM Cubes** (e.g., `OHLC`, `TokenTradeStats`) | Standard time-series analysis, OHLC charts, volume over time | Fast — reads pre-computed minute-level rollups |
...
<AccordionGroup>
  <Accordion title="I need a candlestick chart">
    Use **OHLC** (DWM). It already has pre-computed open/high/low/close/volume per minute. No need to aggregate DEXTrades yourself.
  </Accordion>
  <Accordion title="I need buy/sell counts per token">
    Use **TokenTradeStats** (DWM). It pre-aggregates trade counts, volumes, and unique traders per minute.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Data Cubes" icon="cubes" href="/en/graphql/schema/cubes">
    Explore all 12 Cubes and their field structures.
  </Card>
