...
```javascript
const response = await fetch('https://api.chainstream.io/v2/kyt/transfer', {
  method: 'POST',
  headers: {
  ...
  <Step title="Get Risk Assessment">
    ...
    async function getTransferSummary(transferId) {
      const response = await fetch(
        `https://api.chainstream.io/v2/kyt/transfers/${transferId}/summary`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`
          }
...
<Tabs>
  ...
  <Tab title="Get Detailed Info">
    ...
    const exposures = await fetch(
      `https://api.chainstream.io/v2/kyt/transfers/${transferId}/exposures/direct`,
      { headers: { 'Authorization': `Bearer ${accessToken}` } }
    );

    // Get risk alerts
    const alerts = await fetch(
      `https://api.chainstream.io/v2/kyt/transfers/${transferId}/alerts`,
      { headers: { 'Authorization': `Bearer ${accessToken}` } }
    );
    ```
  </Tab>
...
class ComplianceService {
  ...
  async checkDeposit(deposit) {
    ...
    const registerResponse = await fetch('https://api.chainstream.io/v2/kyt/transfer', {
      method: 'POST',
      headers: {
...
class ComplianceService:
    ...
    def check_deposit(self, deposit: dict) -> dict:
        ...
        register_response = requests.post(
            f'{self.base_url}/v2/kyt/transfer',
            headers=headers,
            json={
                'network': deposit['network'],
                'asset': deposit['asset'],
    ...
    def wait_for_analysis(self, token, transfer_id, max_attempts=10):
        ...
        for _ in range(max_attempts):
            response = requests.get(
                f'{self.base_url}/v2/kyt/transfers/{transfer_id}/summary',
                headers=headers
            )
...
async function handleWithdrawal(request) {
  ...
  // 1. Register address
  const registerResponse = await fetch('https://api.chainstream.io/v2/kyt/address', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
  ...
  const riskResponse = await fetch(
    `https://api.chainstream.io/v2/kyt/addresses/${toAddress}/risk`,
    { headers: { 'Authorization': `Bearer ${token}` } }
  );
