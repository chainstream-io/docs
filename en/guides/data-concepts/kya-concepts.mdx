...
<Steps>
  ...
  <Step title="Register Address">
    ```bash
    POST https://api.chainstream.io/v2/kyt/address
  ...
  <Step title="Get Risk Assessment">
    ```bash
    GET https://api.chainstream.io/v2/kyt/addresses/{address}/risk
    Authorization: Bearer <access_token>
    ```
  </Step>
...
async function batchScreenAddresses(addresses) {
  ...
  for (const address of addresses) {
    // 1. Register address
    await fetch('https://api.chainstream.io/v2/kyt/address', {
      method: 'POST',
      headers: {
    ...
    const riskResponse = await fetch(
      `https://api.chainstream.io/v2/kyt/addresses/${address}/risk`,
