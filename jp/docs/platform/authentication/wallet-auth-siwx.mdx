...
Sign-In with X (SIWX) を使用すると、各 API リクエストでウォレットによるメッセージ署名で ChainStream に認証できます — API Key や OAuth トークンは不要です。[x402 支払い](/jp/guides/getting-started/x402-payments)でサブスクリプションを購入した**オンチェーンウォレットを持つ AI エージェント**向けに設計されています。
...
詳細は [x402 支払い](/jp/guides/getting-started/x402-payments)を参照してください。
