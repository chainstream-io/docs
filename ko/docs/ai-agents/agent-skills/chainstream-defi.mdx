...
description: '비가역적 DeFi 실행 — 스왑, 브릿지, 런치패드, 트랜잭션 브로드캐스트'
icon: "right-left"
---
...
`chainstream-defi` 스킬은 Solana, BSC, Ethereum 전반에 걸친 온체인 DeFi 실행을 처리합니다. 토큰 스왑, 크로스체인 브릿지, 런치패드 토큰 생성, 트랜잭션 브로드캐스트를 지원합니다.
...
| 402 결제 필요 | CLI가 [x402 결제](/ko/guides/cli/x402-payment)를 통해 자동 처리 |
