...
<CardGroup cols={3}>
  ...
  <Card title="chainstream-graphql" icon="diagram-project" color="#10B981">
    **Tool 패턴** — 27개 큐브에 걸친 커스텀 GraphQL 분석: 큐브 간 JOIN, 집계, 시계열
  </Card>
  <Card title="chainstream-defi" icon="right-left" color="#9333EA">
    **Process 패턴** — 비가역적 DeFi 실행: 스왑, 런치패드, 트랜잭션 브로드캐스트
  </Card>
</CardGroup>
...
| 항목 | chainstream-data | chainstream-graphql | chainstream-defi |
|--------|------------------|---------------------|------------------|
| 패턴 | Tool (읽기 전용) | Tool (읽기 전용) | Process (실행) |
| 위험 수준 | 낮음 | 낮음 | 높음 (비가역적) |
| 지갑 필요 여부 | 아니오 (API Key만으로 충분) | 아니오 (API Key만으로 충분) | 예 (서명 필요) |
| MCP 지원 | 전체 (17개 도구) | CLI 기반 | 도구 사용 가능하지만 실행 시 호스트에 지갑 필요 |
| 사용자 확인 | 불필요 | 불필요 | 모든 트랜잭션 전 **필수** |
| 대표 작업 | 검색, 분석, 추적, 스트리밍 | JOIN, 집계, 시계열, 복합 WHERE | 스왑, 토큰 생성, 브로드캐스트 |
| 적합 용도 | 사전 구축 엔드포인트를 통한 표준 분석 | REST가 노출하지 않는 커스텀 분석 | 트레이딩, 토큰 런칭, 서명 |

## 공유 리소스

모든 Skills가 공통 참조 문서를 공유합니다:
...
설정 방법은 [설치 가이드](/ko/docs/ai-agents/agent-skills/installation)를 참조하세요.

## 다음 단계

<CardGroup cols={2}>
  <Card title="설치" icon="download" href="/ko/docs/ai-agents/agent-skills/installation">
    플랫폼에 Skills 설정하기
  </Card>
  ...
  <Card title="chainstream-graphql" icon="diagram-project" href="/ko/docs/ai-agents/agent-skills/chainstream-graphql">
    커스텀 GraphQL 분석, JOIN, 집계
  </Card>
  <Card title="chainstream-defi" icon="right-left" href="/ko/docs/ai-agents/agent-skills/chainstream-defi">
    DeFi 실행 워크플로우
  </Card>
  <Card title="MCP Server" icon="plug" href="/ko/docs/ai-agents/mcp-server/introduction">
    기반 MCP 프로토콜
  </Card>
</CardGroup>
