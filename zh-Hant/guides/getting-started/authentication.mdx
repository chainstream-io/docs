...
適用於擁有鏈上錢包並透過 [x402 支付](/zh-Hant/guides/getting-started/x402-payments) 購買了訂閱的 AI Agent。使用 **Sign-In with X (SIWX)** 標準（EVM 為 EIP-4361，Solana 為等效協議）。
...
<Note>
SIWX 認證需要有效的 x402 訂閱。如果訂閱已過期，請求將被拒絕。參見 [x402 支付](/zh-Hant/guides/getting-started/x402-payments) 瞭解如何購買訂閱。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="什麼是 402 Payment Required 響應？">
    當沒有找到有效憑證且啟用了 x402 時，閘道器返回 HTTP 402 並附帶購買訂閱的指引（`/x402/purchase`）。這使 AI Agent 能夠自動購買訪問許可權。參見 [x402 支付](/zh-Hant/guides/getting-started/x402-payments)。
  </Accordion>
