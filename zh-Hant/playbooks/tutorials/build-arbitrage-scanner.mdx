...
<Info>
ChainStream 目前支援 **Solana**（`sol`）、**Ethereum**（`eth`）和 **BSC**（`bsc`）。支援的 DEX 包括 Jupiter、Raydium、PumpFun、Moonshot、Candy（Solana）以及 KyberSwap（Ethereum/BSC）。以下部分程式碼示例引用了其他 DEX 用於概念說明，請檢視[支援的鏈](/zh-Hant/guides/getting-started/supported-chains)瞭解當前覆蓋範圍。
</Info>
...
<CardGroup cols={2}>
  <Card title="DeFi 監控概述" icon="landmark" href="/zh-Hant/playbooks/frameworks/defi-monitoring-overview">
    瞭解 DeFi 監控維度
  </Card>
